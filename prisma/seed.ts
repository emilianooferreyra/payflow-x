...
async function main() {
  ...
  console.log(
    `📈 USD/ARS: ${usdToArs.toFixed(2)} | USD/BRL: ${usdToBrl.toFixed(2)}\n`,
  );

  // ── Clean existing demo data ──────────────────────────────────────────────
  const existing = await prisma.user.findUnique({
    where: { email: "emiliano@craftbeats.dev" },
  });
  if (existing) {
  ...
  const usdWallet = await prisma.wallet.create({
    data: { userId: user.id, currency: "USD", balance: 1842.5 },
  });
  ...
  for (let i = 0; i < 30; i++) {
    ...
    exchangeRateData.push(
      { fromCurrency: "USD", toCurrency: "ARS", rate: arsRates[i], date },
      {
        fromCurrency: "ARS",
        toCurrency: "USD",
        rate: parseFloat((1 / arsRates[i]).toFixed(8)),
        date,
      },
      ...
      { fromCurrency: "USDT", toCurrency: "USD", rate: 0.9998, date },
      {
        fromCurrency: "USDT",
        toCurrency: "ARS",
        rate: parseFloat((arsRates[i] * 0.9998).toFixed(4)),
        date,
      },
  ...
  console.log(
    "✅ Exchange rates: 30 days (USD/ARS, USD/USDT, USDT/ARS, USD/BRL)",
  );
  ...
  txs.push({
    ...
    metadata: {
      rate: arsRate90,
      received: (1000 * arsRate90).toFixed(2),
      toCurrency: "ARS",
    },
  ...
  txs.push({
    ...
    metadata: {
      rate: arsRate60,
      received: (1500 * arsRate60).toFixed(2),
      toCurrency: "ARS",
    },
  ...
  txs.push({
    ...
    metadata: {
      rate: arsRate30,
      received: (1000 * arsRate30).toFixed(2),
      toCurrency: "ARS",
    },
  ...
  console.log(
    `✅ Transactions: ${txs.length} operations (3 months of history)`,
  );
  ...
  console.log(
    `✅ Yield: ${yieldTxs.length} daily accruals (90 days × USD + USDT)`,
  );
    ...
    data: [
      {
        ...
        quantity: 3.5,
        avgBuyPrice: 182.0,
        currentValue: parseFloat((3.5 * 195.5).toFixed(2)),
      },
      {
        userId: user.id,
        ...
        quantity: 0.8,
        avgBuyPrice: 720.0,
        currentValue: parseFloat((0.8 * 875.4).toFixed(2)),
      },
      {
        userId: user.id,
        ...
        quantity: 2.0,
        avgBuyPrice: 510.0,
        currentValue: parseFloat((2.0 * 548.6).toFixed(2)),
      },
      {
        userId: user.id,
        ...
        quantity: 1.5,
        avgBuyPrice: 390.0,
        currentValue: parseFloat((1.5 * 418.2).toFixed(2)),
      },
    ],
  });
...
const BROKERS = [
  {
    ...
    sources: [
      {
        label: "Tarifas oficiales IOL",
        url: "https://www.invertironline.com/tarifas",
      },
      {
        label: "Rankia (act. 30/04/2026)",
        url: "https://www.rankia.com.ar/blog/analisis-merval/4309655-invertir-online-productos-comisiones-cotizaciones",
      },
    ],
  },
  {
    ...
    sources: [
      {
        label: "Aranceles oficiales Balanz",
        url: "https://balanz.com/comisiones/",
      },
      {
        label: "Rankia (12/08/2025)",
        url: "https://www.rankia.com.ar/blog/trading-argentina/6944904-tarifario-comisiones-balanz-capital",
      },
    ],
  },
  {
    ...
    feeNotes:
      "Rango 0,60%–1,50% según acuerdo; intradía bonifica la pata menor",
    sources: [
      {
        label: "Comisiones oficiales PPI",
        url: "https://portfoliopersonal.com/Contenido/comisiones",
      },
      {
        label: "Rankia (20/09/2025)",
        url: "https://www.rankia.com.ar/blog/trading-argentina/6986316-tarifario-comisiones-ppi",
      },
    ],
  },
  {
    ...
    ivaOnFees: true,
    custodyNotes:
      "Gratis hasta USD 200.000; $1.000/mes hasta USD 300.000; proporcional CVSA por encima",
    feeNotes:
      "Plan base. Cocos Gold (hasta 0,15% AUM/mes) y Pro (desde USD 150/mes) operan al 0%. Operador humano: 1%",
    sources: [
      {
        label: "Tarifario oficial Cocos",
        url: "https://cocos.capital/tarifario",
      },
      {
        label: "Rankia (act. 05/2026)",
        url: "https://www.rankia.com.ar/blog/trading-argentina/7336870-tarifario-comisiones-cocos-capital",
      },
    ],
  },
  {
    ...
    feeNotes:
      "Cuenta Digital. Active Trader 0,25% / AT Plus 0,10% por volumen mensual; intradía −50%",
    sources: [
      {
        label: "Guía oficial de comisiones",
  ...
  },
  {
    ...
    subscriptionNotes:
      "Plan Investor: $5.000 + IVA por mes, solo los meses en que operás mercado local. Plan Rookie (gratis) NO incluye acciones/CEDEARs",
    custodyNotes: "Custodia incluida, mantenimiento bonificado",
    feeNotes: "0% de comisión por operación bajo modelo de suscripción",
    sources: [
      {
        label: "Preguntas frecuentes IEB+",
        url: "https://www.iebmas.com.ar/preguntas-frecuentes/",
      },
    ],
  },
];
...
async function seedBrokers() {
  ...
  console.log(
    `✅ Brokers: ${BROKERS.length} tarifarios (verificados ${BROKER_VERIFIED_AT.toISOString().slice(0, 10)})\n`,
  );
}
