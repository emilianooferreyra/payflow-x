...
describe("HashService (e2e)", () => {
  ...
  describe("verify", () => {
    it("should verify a known password against its pre-computed hash", async () => {
      await expect(
        hashService.verify(KNOWN_HASH, KNOWN_PASSWORD),
      ).resolves.toBe(true);
    });
    ...
    it("should verify a freshly hashed password correctly", async () => {
      ...
      await expect(hashService.verify(hashed, KNOWN_PASSWORD)).resolves.toBe(
        true,
      );
    });

    it("should reject a similar but wrong password", async () => {
      await expect(hashService.verify(KNOWN_HASH, "1234568")).resolves.toBe(
        false,
      );
    });
    ...
    it("should reject the correct password against a different hash", async () => {
      ...
      await expect(hashService.verify(otherHash, KNOWN_PASSWORD)).resolves.toBe(
        false,
      );
    });
  });
